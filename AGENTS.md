...
- 이 파일 = **항상 지켜야 하는 규칙.** 잘 바뀌지 않습니다.
- `TASKS.md` = **지금 해야 할 일 목록.** 작업을 끝내면 그 항목을 지웁니다.

무엇을 할지 지시받지 않았다면 `TASKS.md`의 1번부터 보세요.
...
**`TASKS.md`를 보세요.** 남은 작업과 알려진 빚은 전부 거기 있습니다.
같은 내용을 이 파일에 옮겨 적지 마세요. 두 벌이 되면 반드시 어긋납니다.
(이 저장소에는 이미 그런 사고가 있습니다 — 개인정보처리방침이 두 벌인데
서로 다르고, 앱은 옛 쪽을 보여주고 있습니다.)

지금 상태를 오해하지 않도록 두 가지만 미리 알려둡니다.

- `app/tennis-note-legal/terms.html`이 **없습니다.** 가입 동의 화면이 이 파일을
  링크하고 있어 404입니다. 링크 검사는 `KNOWN_MISSING_PAGES`로 통과시키고 있습니다.
- **파일이 큽니다.** `app/admin/app.js` 31,096줄, `bindEvents()` 하나가 2,385줄.
  전체를 읽으려 하지 말고 필요한 부분만 찾아 읽으세요.
