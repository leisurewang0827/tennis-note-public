...
- 모든 기능 개발·버그 수정·구조 정리는 `dev` 에서 시작하고 끝냅니다.
- `dev` 에서 `./scripts/verify.sh` 와 관련 브라우저 확인을 모두 통과해야 합니다.
- `main` 은 현재 운영에 배포된 코드만 둡니다. 직접 커밋하거나 직접 푸시하지 않습니다.
- 배포는 운영자가 요청했을 때만 버전을 별도 커밋으로 올리고 `dev` → `main` PR 로 진행합니다.
- `main` 병합은 필수 검사 통과와 운영자의 배포 승인이 모두 확인된 뒤에만 합니다.
- 끝나지 않은 작업이 `dev` 에 있을 때 긴급 수정이 필요하면 `main` 에서 별도 hotfix 브랜치를 만듭니다.
- 원격 기본 브랜치·보호 규칙 같은 저장소 설정도 운영자 승인 없이 변경하지 않습니다.

`main` push 는 Cloudflare Pages와 GitHub Pages 운영 배포를 바로 시작합니다.
따라서 **PR 병합과 배포는 같은 승인 경계**입니다.
...
**왜 이 저장소에서 특히 중요한가**: `app.js` 를 160여 개 파일로 나눠서 버전
올리기가 **350곳 넘게** 건드립니다. 한 커밋에 섞으면 20줄짜리 기능 변경이
