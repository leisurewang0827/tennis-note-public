  ...
  function bind(options) {
    ...
    async function run(file) {
      ...
      try {
        ...
        const remoteDisabledReason = remote && previewTransport.enabled !== true
        ...
        worker.onmessage = async event => {
          ...
          if (remoteDisabledReason && !event.data.result?.errors?.includes("EMPTY_DATA")) {
        ...
        worker.postMessage({ id, bytes, snapshot, now: new Date().toISOString(), ...(local ? { serverProtocol: "local-synthetic/1" } : remoteEnabled ? { serverProtocol: previewTransport.protocol } : {}) }, [bytes]);
