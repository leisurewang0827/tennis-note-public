...
시작할 때는 각각 14,218 · 7,998 · 31,096줄이었습니다. **`app.js` 에는
`state` 와 부팅 코드, `bindEvents`, `initApp`(코치는 `initCoachApp`) 만
남았습니다.**

**아직 안 나눈 파일이 둘 있습니다.**

- `app/admin/schedule-v2-admin.js` (4,144줄) — 함수 156개가 전부 IIFE
  안에 있습니다. 구조가 달라 위 방법이 그대로 통하지 않습니다.
- `app/shared/tennisnote-curriculum-catalog.js` (4,562줄) — 함수 0개,
  **순수 데이터 표**입니다. 나눌 것이 없으니 그대로 두세요.
...
로컬에서 확인할 때 함수가 "없다" 고 나오면 십중팔구 이것입니다. 서비스워커
얘기가 아니라 평범한 HTTP 캐시입니다. 콘솔에서 아래를 돌리고 새로고침하세요.
...
저쪽은 하루 3~4번 배포합니다. **미루지 말고 자주 하세요.** 며칠 몰아두면
한 번에 어려워집니다.

```bash
git fetch origin
./scripts/merge_report.py        # 아무것도 바꾸지 않습니다. 먼저 보세요.
```

이 보고서가 **"저쪽이 고친 함수 → 우리 쪽 현재 위치"** 표를 만들어 줍니다.
우리는 app.js 를 쪼갰으므로 충돌 마커만 보고 풀면 오판합니다.

푸는 방식은 하나입니다 — **저쪽 app.js 를 통째로 가져와서, 우리가 다른
파일로 옮긴 것만 다시 빼냅니다.** 그러면 저쪽 변경을 하나도 안 놓칩니다.
충돌 블록을 손으로 고르지 마세요.

⚠ **저쪽 본문과 우리 본문이 다르다고 바로 바꾸면 안 됩니다.** 우리 쪽에는
기본값 이음매가 102개 남아 있어서, 그렇게 하면 그 작업이 통째로 되돌아갑니다.
**기준점(`git merge-base`)과 비교해서 저쪽이 실제로 고친 것만** 손대세요.
(실제로 이걸 빠뜨려 한 번 되돌렸습니다.)

`bindEvents` 는 우리가 `events/` 로 쪼개서 통째 대조가 안 됩니다. 저쪽이
넣은 덩어리를 문맥으로 찾아 제자리에 넣고, **우리 `events/` 전체를 이으면
저쪽 `bindEvents` 본문과 같은 줄들인가** 로 확인하세요 (순서는 다릅니다).

`index.html` 과 서비스워커는 **우리 스크립트 목록을 유지**하고 저쪽 신규
스크립트만 더합니다. 버전과 캐시 이름은 저쪽 것을 씁니다.

끝나고 확인할 것: **저쪽 함수 유실 0 · 중복 0** · `./scripts/verify.sh` ·
**브라우저**.
...
- `app/admin/app.js`(1,013줄)를 더 키우지 마세요. 새 기능은 별도 파일로.

## 미리 알아둘 것

지금 상태를 오해하지 않도록. 고칠 때 이 목록도 같이 지우세요.

- **`app/admin/schedule-v2-admin.js`(4,144줄)는 아직 안 나눴습니다.**
  함수 156개가 IIFE 안에 있어 다른 파일들과 구조가 다릅니다.
