...
세 앱을 역할별로 나누는 중입니다. 회원앱·코치앱은 끝났고 관리자는 절반입니다.
각 폴더가 뜻하는 것은 아래 "큰 파일을 쪼개는 방법" 에 있습니다.
...
| `data/` | 5개 1,779줄 | 4개 1,246줄 | — |
| `actions/` | 6개 1,566줄 | 4개 1,070줄 | — |
| `ui/` | 2개 768줄 | 2개 443줄 | — |
| `storage.js` | 338줄 | 209줄 | — |
| `catalog.js` | 410줄 | 55줄 | — |
| `settings.js` | 61줄 | 47줄 | — |
| **`app.js`** | **3,763줄** | **721줄** | **21,520줄** |
...
**정규식으로 JS 를 정적 분석하다 지금까지 여섯 번 오분류했습니다.** 분류는
...
- `app/admin/app.js`(21,520줄)를 더 키우지 마세요. 새 기능은 별도 파일로.

## 미리 알아둘 것

지금 상태를 오해하지 않도록. 고칠 때 이 목록도 같이 지우세요.

- **`app/admin/app.js` 가 아직 큽니다(21,520줄).** 통째로 읽을 수 없으니
  필요한 부분만 찾아 읽으세요. 회원앱·코치앱에 적용한 `data/`·`actions/`·
  `ui/`·`storage.js` 계층이 관리자에는 아직 없습니다. 같은 절차로 약 9,100줄을
  더 덜어낼 수 있습니다.
